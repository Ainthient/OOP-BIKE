...
namespace BIKESobj.DesktopClient
{
    ...
    public partial class App : Application
    {
        ...
        {
            services.AddSingleton<IDomainObjectsCache<bikeobj>, DomainObjectsMemoryCache<bikeobj>>();
            services.AddSingleton<NetworkBIKESobjRepository>(
                x => new NetworkBIKESobjRepository("localhost", 80, useTls: false, x.GetRequiredService<IDomainObjectsCache<bikeobj>>())
            );
            services.AddSingleton<CachedReadOnlyBIKESobjRepository>(
                x => new CachedReadOnlyBIKESobjRepository(
                    ...
                    x.GetRequiredService<IDomainObjectsCache<bikeobj>>()
                )
            );
